...
int main(int argc, char** argv) {
  ...
  std::cerr << "ocr_det: C++ " << boxes.size() << " boxes vs golden " << want.size() << "\n";
  ...
  for (auto& db : boxes) {
    ...
    else std::cerr << "  unmatched C++ box: [" << a.x0 << "," << a.y0 << ","
                   << a.x1 << "," << a.y1 << "] (nearest d=" << bestd << ")\n";
  }
  for (size_t j = 0; j < want.size(); ++j)
    if (!used[j]) std::cerr << "  missed golden box: [" << want[j].x0 << "," << want[j].y0
                            << "," << want[j].x1 << "," << want[j].y1 << "]\n";

  CHECK_MSG(boxes.size() == want.size(), "box count");
  CHECK_MSG(matched == (int)want.size(), "all boxes matched within tol");
  std::cerr << "ocr_det: " << matched << "/" << want.size() << " boxes match golden (tol "
            << TOL << "px)\n";
  return TEST_SUMMARY();
}
