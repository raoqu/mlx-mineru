...
8. **每次推进提交 Git**：每完成一次推进（一个阶段或一个可独立验证的子环节）后，立即 `git commit`，提交信息写清"做了什么 + 如何验证 + 验证结果"，使进度可追溯、可回滚。绝不在未验证通过时提交。提交时**不要**把 AI/Claude 添加为作者或 `Co-Authored-By`（提交署名只保留人类作者）。
...
- **多页 + 标准输出 + 服务器 ✅**：`-s/-e` 页范围；输出 MinerU 布局 `output/<name>/vlm/{.md,_content_list.json,_middle.json}`。`--server`（vendored cpp-httplib）：`GET /health`、`POST /file_parse` → `{md_content, content_list}`。
- **内容类型覆盖 ✅**：text（+post_process 文本链）、title、**table（OTSL→HTML，ctest `otsl`）**、equation（+post_process 公式链，ctest `post_process`）、**image/chart（stb 存 JPEG + image_path + 分析 details）**。header/footer/page_number/page_footnote/aside→discarded。
- **剩余保真工作（非阻塞，已可用）**：(1) title 标题层级（当前固定 1）；(2) MagicModel 完整 caption/footnote 关联与块归类；(3) 其余 5 个公式修复函数（left_right/big/leq/eqqcolon/double_subscript）；(4) image_analysis 重分类（pure_table/pure_formula/chart）；(5) cross-page table merge；(6) 与 MinerU Python 逐字节对齐（需可跑其 VLM）；(7) office/pipeline 后端（低优先）。
- **pipeline-MNN：Metal 加速 + MFR KV-cache ✅**：MNN 静态库改为 `MNN_METAL=ON MNN_COREML=ON`（Metal shader 内嵌源码编译，仍无外部 metallib、二进制可重定位）；运行时由 `MINERU_MNN_BACKEND`(cpu|metal|coreml|auto) 选后端、**默认 Metal**、fp32 保精度、非 CPU 后端加载/首次推理失败自动回退 CPU（`mnn_runner.cpp` + CMake 链接 Metal/CoreGraphics/CoreML/CoreVideo）。Metal vs CPU(M3 Ultra)：unet ~12×、ocr_rec ~7×、ocr_det ~1.7×、table_cls ~1.2×。MFR：Swin 编码器走 Metal（~6.5×，`formula_rec.cpp` 用 `mfr_encoder.mnn` sibling，否则 ORT）；mBART 解码器重导出为**合并 KV-cache 图**（`export_mfr_onnx.py`，单份权重 ~577MB，自注意力缓存、交叉注意力每步重算）→ ORT 跑、与后端无关，197-token 公式 ~3.7× 且贪心 ids 精确。CoreML 三模型 codegen 失败、layout(While/CumSum 发散)、slanet(GRU Loop 即便修好 Concat 仍 0.87× 慢) 均留 ORT。验证：`ctest` 全 27 项在默认 Metal 下通过；`mfr` golden 在 CPU/Metal 均 exact ids。
